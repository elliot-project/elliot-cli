...
class SpuriousUrbanCarsTask(BaseTask):
    ...
    @property
    def dataset_specs(self) -> list[DatasetSpec]:
        # UrbanCars is composited locally from Stanford Cars, Places and LVIS
        # rather than published as a dataset, so there is nothing to pre-stage
        # from the Hub. The image tree is supplied via URBANCARS_DATA_DIR.
        return []
